...
for run in INCIDENT_PHOTON_ENERGY.keys():
    ELASTIC_PEAK[run] = (
        7.8 + INCIDENT_PHOTON_ENERGY[run] - 774 + ANALYZER_KINETIC_ENERGY[run] - 698.7
    )
...
def get_recorded_au4f_spectrum():
    ...
    psf_kinetic_energy = (
        np.arange(len(psf_intensity)) + 100
    ) * PIXELS_TO_EV + ANALYZER_KINETIC_ENERGY["17"]
    psf_binding_energy = INCIDENT_PHOTON_ENERGY["17"] - psf_kinetic_energy
    psf = {
        "x": psf_kinetic_energy,
        "y": psf_intensity,
...
def get_extended_au4f_spectrum(energy_spacing, num_points):
    ...
    extended_au4f = {
        "x": energies,
        "y": np.interp(energies, psf["x"], psf["y"], left=left, right=right),
        "binding_energy": psf_binding_energy,
    }
