...
def dummy_convolved_data():
    ...
    b2 = 0.15
    impulse_response_y = 5*(b2)/((impulse_response_x-368.3)**2+(b2)**2)+3*(b2)/((impulse_response_x-374.0)**2+(b2)**2)
    impulse_response_y = impulse_response_y/np.sum(impulse_response_y)
    noiseless_convolved_y = convolve(
        ground_truth_y,
        impulse_response_y,
        mode='valid'
    )
    convolved_x = np.linspace(0, spacing*(len(noiseless_convolved_y)-1), len(noiseless_convolved_y))
    num_spectra = 500
    ...
    for _ in range(num_spectra):
        current = np.random.poisson(noiseless_convolved_y/single_photon)*single_photon
...
def test_single_LR_iteration(dummy_convolved_data):
    ...
    initial_guess = initial_guess*np.sum(dummy_convolved_data['ground_truth_y'])/np.sum(initial_guess)
    ...
    blurred = convolve(dummy_convolved_data['impulse_response_y'], initial_guess, mode="valid")
